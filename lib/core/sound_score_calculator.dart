...
class SoundScoreResult {
  ...
  final int flatness;      // 0–40  (200Hz–8kHz 표준편차)
  final int bassExt;       // 0–20  (40Hz 저역 extension)
  final int trebleRolloff; // 0–20  (10k→20k 롤오프 자연스러움)
...
/// 주파수 응답 bins → 0-100 Sound Score 계산 (클라이언트 측 rule-based)
class SoundScoreCalculator {
  ...
  static SoundScoreResult? compute(List<FrequencyBin>? bins) {
    ...
    const channelMatch = 15; // stereo 측정 없이 기본값
  ...
  static int _flatnessScore(List<FrequencyBin> bins) {
    ...
    // std ≤ 3dB → 40pts, std ≥ 15dB → 0pts
    return (40.0 * (1.0 - (std - 3.0).clamp(0.0, 12.0) / 12.0))
        .round()
        .clamp(0, 40);
  }

  // 40Hz 기준 저역 extension → 0–20점
  static int _bassScore(List<FrequencyBin> bins) {
    ...
    // delta ≥ -3dB → 20pts, delta ≤ -20dB → 0pts
    return (20.0 * (delta + 20.0).clamp(0.0, 17.0) / 17.0)
        .round()
        .clamp(0, 20);
  }

  // 10kHz → 20kHz 롤오프 자연스러움 → 0–20점
  static int _trebleScore(List<FrequencyBin> bins) {
    ...
    final drop = at10k - at20k; // positive = 롤오프
    if (drop >= 2 && drop <= 8) return 20; // natural rolloff
...
/// 측정 스펙트럼으로부터 클라이언트 측 Sound Score를 반환하는 Provider
