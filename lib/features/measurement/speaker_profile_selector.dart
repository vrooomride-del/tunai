  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          const Text('SPEAKER SETUP', style: TextStyle(color: Colors.white70, fontSize: 11, letterSpacing: 3)),
          const SizedBox(height: 4),
          const Text('스피커 유형을 선택하면 TUNAI가 최적 범위 안에서 소리를 조정합니다.', style: TextStyle(color: Colors.white38, fontSize: 11)),
          ...
          _ModeCard(title: '직접 입력', subtitle: '스피커 사양 직접 입력', selected: _mode == SpeakerProfileMode.custom, icon: Icons.tune, onTap: () => setState(() => _mode = SpeakerProfileMode.custom)),
          const SizedBox(height: 8),
          _ModeCard(title: '기본 설정으로 시작', subtitle: '스피커 정보 없이 시작', selected: _mode == SpeakerProfileMode.skip, icon: Icons.skip_next, onTap: () => setState(() => _mode = SpeakerProfileMode.skip)),
          if (_mode == SpeakerProfileMode.custom) ...[
            const SizedBox(height: 16),
            const Divider(color: Colors.white12),
            const SizedBox(height: 12),
            const Text('SPEAKER SPECS', style: TextStyle(color: Colors.white38, fontSize: 10, letterSpacing: 2)),
            const SizedBox(height: 12),
            Row(children: [
          ...
          if (_mode != SpeakerProfileMode.skip) ...[
            ...
            const Divider(color: Colors.white12),
            const SizedBox(height: 12),
            const Text('음향 데이터 (선택)', style: TextStyle(color: Colors.white38, fontSize: 10, letterSpacing: 2)),
            const SizedBox(height: 4),
            const Text('음향 데이터가 있으면 더 정확한 소리 최적화가 가능합니다.',
          ...
          ElevatedButton(
            ...
            child: Text(_mode == SpeakerProfileMode.skip ? '기본 설정으로 시작' : '이 설정으로 시작',
...
class _ProfileInfo extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('스피커 보호 범위 (자동 적용)', style: TextStyle(color: Colors.white38, fontSize: 10, letterSpacing: 1.5)),
      const SizedBox(height: 8),
      _InfoRow('저역 보호', '${profile.recommendedHpfFreq.toStringAsFixed(0)} Hz'),
      _InfoRow('최대 저역 강조', '${profile.maxBassBoostDb.toStringAsFixed(1)} dB'),
      _InfoRow('출력 기준', '${profile.gainReferenceOffset >= 0 ? '+' : ''}${profile.gainReferenceOffset.toStringAsFixed(1)} dB'),
    ]),
  );
}
